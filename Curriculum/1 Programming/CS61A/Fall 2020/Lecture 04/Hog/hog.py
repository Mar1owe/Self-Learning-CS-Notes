...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    rolled_num = 0
    st = 0  # This is the score of this turn.
    while rolled_num < num_rolls:
        cv = dice()  # This is the current value of dice.
        if cv != 1:
            st += cv
            rolled_num += 1
        else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    dice_num = 1    # This will loop through 1 to 10 to see which one gives the maximum value.
    highest = 0
    while dice_num <= 10:
        average = make_averaged(roll_dice, trials_count)(dice_num, dice)
        if average > highest:
            highest = average
            result = dice_num
        dice_num += 1
